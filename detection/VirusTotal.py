...
VIRUSTOTAL_API_KEY = "24f9e36e525617f07931a3e5e4b18554ce93bb5ec4580b58db7390fba6b53b38"
...
DARK_THEME = {
    ...
    "log_background": "#2E2E2E",

    "log_foreground": "#575656",
...
class KeyloggerScanner(QThread):
    ...
    def __init__(self):
        ...
        self.scanned_hashes = set()  # To prevent duplicate scans
    ...
    def run(self):
        ...
        self.update_log.emit(
            ...
            f"<span style='font-size:16px; font-weight:bold; '>🔍 --> Scanning for suspicious processes...</span>",
        ...
        # Only scan if we have both suspicious processes and hook detection

        if suspicious_processes and hooked:

            self.update_log.emit(

                f"<span style='font-weight:bold; font-size:15px;'> 🚨🚨 KEYLOGGER DETECTED !!!</span>",

                "red")

            # Start VirusTotal analysis

            self.update_log.emit("<span style='color:orange;'>🛡️ Starting VirusTotal analysis...</span>", "orange")

            self.scan_suspicious_modules(suspicious_processes)

            # Start network monitoring

            self.update_log.emit(

                f"<span style='font-weight:bold; font-size:15px;'>🛰️ Monitoring network activity...</span>",
            ...
            self.update_log.emit(
                ...
                f"<span style=' font-size:15px; font-weight:bold;'>✅ NO KEYLOGGER detected. You're SAFE ! </span>",

                "lightgreen")

    def scan_suspicious_modules(self, processes):

        """Scan only the confirmed suspicious modules with VirusTotal"""

        for pid, name, module in processes:

            if module != "N/A" and module not in self.scanned_hashes:
                self.scanned_hashes.add(module)

                self.scan_with_virustotal(module, name)

    # ... (Keep get_loaded_modules, detect_suspicious_processes,

    #      get_file_hash methods the same)

    def scan_with_virustotal(self, file_path, process_name):

        if not VIRUSTOTAL_API_KEY:
            self.update_log.emit("⚠️ VirusTotal API key missing!", "orange")
            ...
            if response.status_code == 200:
                ...
                if malicious > 0:
                    ...
                    engines = [
                        ...
                        k for k, v in result['data']['attributes']['last_analysis_results'].items()
                    ...
                    self.update_log.emit(
                        ...
                        f"🛡️ VirusTotal: {process_name} detected by {malicious} engines: {', '.join(engines[:3])}...",
                    ...
                    self.update_log.emit(
                        ...
                        f"🛡️ VirusTotal: {process_name} - No malicious indicators found",
                ...
                self.update_log.emit(f"⚠️ VirusTotal error: {response.status_code}", "orange")


        except requests.exceptions.RequestException as e:

            self.update_log.emit(f"⚠️ Network error during VirusTotal scan: {str(e)}", "orange")

        except Exception as e:

            self.update_log.emit(f"⚠️ VirusTotal scan failed: {str(e)}", "orange")
...
def monitor_network_activity(self, detected_processes):
    ...
    self.update_log.emit(
        f"<span style='font-weight:bold; font-size:15px; '>🛰️ Monitoring OUTGOING Network Activity to unknown servers...</span>",
        "lightgreen")
...
class KeyloggerGUI(QWidget):
    ...
    def setup_scan_tab(self):
        ...
        self.status_label = QLabel(
            f"<span style=' font-size:19px; color:whitesmoke; font-family:Times; '>Click ' START SCAN ' to check for KEYLOGGERS </span>")
        ...
        if "NO KEYLOGGER detected" in message:
            ...
            QTimer.singleShot(2000, lambda: (
                ...
                self.status_label.setText(
                    f"<span style='font-size:20px; color:lightgreen; font-weight:bold;'>✅ SCAN COMPLETE... No threats found. Click on <span style='color:white;'>' Detection Results '</span> for more clear analysis !!!  </span>"),
                self.scan_button.setEnabled(True),
                self.stop_button.setEnabled(False)
            ))
        elif "KEYLOGGER DETECTED" in message:
            self.progress_dialog.update_status(
                f"<span style=' font-size:15px; font-weight:bold; '>🚨 Keylogger detected , KILL IT !!! </span>", "red")
            self.movie.stop()
    ...
    def terminate_process(self, row, pid, name):
        try:
            ...
            if not self.active_keylogger_pids:
                self.scanner_thread.running = False
                self.status_label.setText(
                    f"<span style='font-size:15px; '>✅ No more suspicious activity detected.</span>")
